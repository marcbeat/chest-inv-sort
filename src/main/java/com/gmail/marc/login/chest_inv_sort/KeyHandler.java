...
public class KeyHandler {
    ...
    public void onKeyPressed(ScreenEvent.KeyPressed.Pre event) {
        ...
        if (openedScreen instanceof ContainerScreen) {
            ...
            isChest = true;
            ChestInvSort.LOGGER.debug("[{}] Container is {}! Checking pressed key...", ChestInvSort.MODID, containerScreen.getMenu().getClass().getSimpleName());
        }
        else if (openedScreen instanceof InventoryScreen) {
            InventoryScreen inventoryScreen = (InventoryScreen) openedScreen;
            ChestInvSort.LOGGER.debug("[{}] Container is {}! Checking pressed key...", ChestInvSort.MODID, inventoryScreen.getMenu().getClass().getSimpleName());
        }
        else return;
        ...
        ChestInvSort.LOGGER.debug("[{}] Pressed key >> {}", ChestInvSort.MODID , GLFW.glfwGetKeyName(event.getKeyCode(), event.getScanCode()));
        if (! ( pressedKey != null && Config.sortKey.toLowerCase().equals(pressedKey) ) ) return;
        
        ChestInvSort.LOGGER.debug("[{}] Correct key pressed! Start sorting...", ChestInvSort.MODID);
